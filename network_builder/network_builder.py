...
def identify_device_template(templates: list, device: dict) -> dict:
    ...
    for template in templates:
        #pdb.set_trace()
        if device["device_role"] in template["device_roles"] and device["device_type"] in template["device_types"]:
            return template

    raise NTMException(f"No template found for: \ndevice_role: {device['device_role']} \ndevice_type: {device['device_type']}")
...
def render_config(template: str, render_vars: dict) -> str:
    ...
    try:
        ...
        env = Environment(loader=FileSystemLoader("./"), 
                        trim_blocks=True, 
                        lstrip_blocks=True, 
                        undefined=DebugUndefined) # undefined=DebugUndefined UndefinedVars
